...
fit.opt.CV <- anchor.regression(X.train, Y.train, A, gamma.opt.CV, n)

b.opt.CV <- coef(fit.opt.CV)
b.opt.CV
...
legend(1.7, 6.5, legend=c("CV MSE", "true MSE", "CV optimal g", "true optimal g"),
...
ggplot(data, aes(y=testMSE)) +
  ...
  geom_vline(xintercept = b.opt.CV, linetype="dotted", 
             color = "blue", size=1)+
  geom_vline(xintercept = b.opt, linetype="dotted", 
