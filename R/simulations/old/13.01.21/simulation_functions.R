...
# sim1(ex1): Rothenhaeusler Comparison ---------------------------------------
...
# Store states and data
path_name <- "C:/Users/maicr/Desktop/Github/MScAnchor/data sets/sim2/sim2_fixi_data_deviance"
...
############# WORKABLE SETTINGS ########################
# Define variables for unperturbed and perturbed data set
def_poi_IV <- defData(varname = "A", dist = "normal", 
                      formula = 1, variance = 0.25)
def_poi_IV <- defData(def_poi_IV, varname = "H", dist = "normal",
                      formula = 0, variance = 0.25)
def_poi_IV <- defData(def_poi_IV, varname = "X", dist = "normal", 
                      formula = "0.6 * H + 5 * A", variance = 0.25)
def_poi_IV <- defData(def_poi_IV, varname = "Y", dist = "poisson", link = "log", 
                      formula = "0.4 * X + 0.5 * H")
causal_parameter <- 0.4

dd <- genData(5000, def_poi_IV)
hist(dd$Y)
hist(0.4*dd$X + 0.5 * dd$H)
############# WORKABLE SETTINGS #####################

#### OTHER
# Define variables for unperturbed and perturbed data set
def_poi_IV <- defData(varname = "A", dist = "normal", 
                      formula = 1, variance = 0.04)
def_poi_IV <- defData(def_poi_IV, varname = "H", dist = "normal",
                      formula = 1, variance = 0.04)
def_poi_IV <- defData(def_poi_IV, varname = "X", dist = "normal", 
                      formula = "2 * H + 1 * A", variance = 0.04)
def_poi_IV <- defData(def_poi_IV, varname = "Y", dist = "poisson", link = "log", 
                      formula = "0.3 * X + 1 * H")
...
onerep_IV <- function(rep, nobs_values = 300, data_table, 
  ...
  for (k in 1:length(nobs_values)) {
    ...
    data_temp <- data.frame(rep = rep, nobs = nobs,
                            xi = c(xi_values, xi_big),
                            b = c(b_glare, b_big),
                            b_se = c(b_glare_se, b_big_se))
...
# Define simulation function
simulate_IV <- function(nsim, nobs_values = 300, xi_values, xi_big = 10000,
...
# sim5: IV investigation for poisson example 2 --------------------------------

# Investigate mean
def_poi_IV <- defData( varname = "Y", dist = "poisson", link = "log", 
                       formula = "4")
dd <- genData(5000, def_poi_IV)
hist(dd$Y)
...
# Simulate
IV_b <- simulate_IV(nsim = nsim, nobs_values = nobs_values,
                    xi_values = xi_values, xi_big = xi_big,
                    data_table = data_table,
                    formula = formula, A_formula = A_formula,
                    family = family, type = type)
...
path_name <- "C:/Users/maicr/Desktop/Github/MScAnchor/data sets/sim_IV/"
dir.create(paste(path_name, Sys.Date(), sep = ""))
save(IV_b, file = paste(paste(path_name, Sys.Date(), sep = ""), "/IV_b_deviance.Rdata", sep =""))

# sim6: IV investigation for binomial -----------------------------------------

# Investigate mean
def_bin_IV <- defData( varname = "Y", dist = "binomial", link = "logit", 
...
def_bin_IV <- defData(varname = "randA", dist = "binary",
                   formula = 0.5)
def_bin_IV <- defData(def_bin_IV, varname = "A", 
                   formula = "2 * randA - 1")



def_bin_IV <- defData(varname = "A", dist = "normal", 
                      formula = 1, variance = 0.25)
def_bin_IV <- defData(def_bin_IV, varname = "H", dist = "normal",
                      formula = 2, variance = 0.25)
def_bin_IV <- defData(def_bin_IV, varname = "X", dist = "normal", 
                      formula = "- 3 * H + 5 * A", variance = 0.25)
