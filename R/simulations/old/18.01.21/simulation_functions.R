...
# Simulate
sim_data_rot_X_fixi <- simulate_fixi(nsim = 100,nobs = 300,
                                     xi_values = xi_values,
                                     v_values = v_values, 
                                     data_table = data_table,
                                     data_pert_table = data_pert_table, 
...
par(mfrow = c(2,3))
hist(0.4*dd$X + 1 * dd$H, breaks = 100)
hist(exp(0.4*dd$X + 1 * dd$H)/(1+exp(0.4*dd$X + 1 * dd$H)), breaks = 100)
hist(dd$Y, breaks = 100)
hist(0.4*dd_pert$X + 1 * dd_pert$H, breaks = 100)
hist(exp(0.4*dd_pert$X + 1 * dd_pert$H)/(1+exp(0.4*dd_pert$X + 1 * dd_pert$H)), breaks = 100)
hist(dd_pert$Y, breaks = 100)
...
def_poi_X_pert <- defData(def_poi_X_pert, varname = "H", dist = "normal",
                          formula = 0, variance = 0.25)
def_poi_X_pert <- defData(def_poi_X_pert, varname = "X", dist = "normal", 
                          formula = "0.8 * H + v * A", variance = 0.25)
...
IV_temp <- simulate_IV(nsim = nsim, nobs_values = nobs_values,
                       xi_values = xi_values,
                       data_table = data_table,
                       formula = formula, A_formula = A_formula,
                       family = family, type = type)
...
load("C:/Users/maicr/Desktop/Github/MScAnchor/data sets/simulation_study/ex2/2021-01-18/smooth/sim2_smooth.Rdata")
