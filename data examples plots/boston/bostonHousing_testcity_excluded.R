...
index <- which(bostonPrices$town %in% perturbation_towns)
train_set <- bostonPrices[-index, ]
fulltest_set <- bostonPrices[index, ]
...
for (i in 1:xi_len) {
  ...
  for (t in 1:length(perturbation_towns)) {
    ...
    test_set <- fulltest_set[fulltest_set$town == pert_town,  ]

    # Predictions, Residuals and MSE
    predictions <- predict(fit_temp, type = "response", newdata = test_set)
    resid_pert <- test_set$log_medv - predictions
    RMSE <- sqrt(mean((resid_pert)^2))
    
    # Log-likelihood
    logLik_indiv <- as.numeric(logLik(fit_temp, newdata = test_set,
                                      indiv = TRUE))
    logLik <- as.numeric(quantile(logLik_indiv, 0.9))
    
    prediction_list[[t]] <- list(xi, pert_town,
                                 predictions, RMSE,
                                 logLik_indiv, logLik)
  }
  
  complete_list[[i]] <- list(xi, prediction_list) 
...
save(complete_list, file = paste(paste(path_name, Sys.Date(), sep = ""), "/complete_list.Rdata", sep =""))

load("C:/Users/maicr/Desktop/Github/MScAnchor/data sets/data_examples/bostonHousing/2021-01-07/complete_list.Rdata")
...
train_set <- bostonPrices[bostonPrices$town != "Somerville", ]
test_set <- bostonPrices[bostonPrices$town == "Somerville", ]

nrow(train_set)
nrow(test_set)

# Initialize and run GLARE
# xi_values <- seq(0, 20, by = 1)
xi_values <- c(0, 0.3, 0.5, 0.8, 1, 1.5, 2, 5, 10, 20, 50, 100)
